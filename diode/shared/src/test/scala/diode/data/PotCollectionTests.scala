  ...
  def tests = TestSuite {
    ...
    'PotStream - {
      'append - {
        ...
        val ps1 = ps.append(Seq("1" -> Ready("test1")))
        assert(ps1.headKeyOption.contains("1") && ps1.lastKeyOption.contains("1"))
        val ps2 = ps1.append(Seq("2" -> Ready("test2")))
        assert(ps2.headKeyOption.contains("1") && ps2.lastKeyOption.contains("2"))
        val ps3 = ps2.append(Seq("3" -> Ready("test3"), "4" -> Ready("test4")))
        assert(ps3.headKeyOption.contains("1") && ps3.lastKeyOption.contains("4"))
        assert(ps3.elems.size == 4)
        assert(ps3.elems("1").next.contains("2"))
        assert(ps3.elems("1").prev.isEmpty)
        assert(ps3.elems("2").next.contains("3"))
        assert(ps3.elems("2").prev.contains("1"))
        assert(ps3.elems("3").next.contains("4"))
        assert(ps3.elems("3").prev.contains("2"))
        assert(ps3.elems("4").next.isEmpty)
        assert(ps3.elems("4").prev.contains("3"))
      }
      'prepend - {
        val fetcher = new TestFetcher[String]
        val ps = PotStream[String, String](fetcher)
        val ps1 = ps.prepend(Seq("1" -> Ready("test1")))
        assert(ps1.headKeyOption.contains("1") && ps1.lastKeyOption.contains("1"))
        val ps2 = ps1.prepend(Seq("2" -> Ready("test2")))
        assert(ps2.headKeyOption.contains("2") && ps2.lastKeyOption.contains("1"))
        val ps3 = ps2.prepend(Seq("4" -> Ready("test4"), "3" -> Ready("test3")))
        // println(s"${ps3.elems} - ${ps3.headKeyOption} - ${ps3.lastKeyOption}")
        assert(ps3.headKeyOption.contains("4") && ps3.lastKeyOption.contains("1"))
        assert(ps3.elems.size == 4)
        assert(ps3.elems("1").prev.contains("2"))
        assert(ps3.elems("1").next.isEmpty)
        assert(ps3.elems("2").prev.contains("3"))
        assert(ps3.elems("2").next.contains("1"))
        assert(ps3.elems("3").prev.contains("4"))
        assert(ps3.elems("3").next.contains("2"))
        assert(ps3.elems("4").prev.isEmpty)
        assert(ps3.elems("4").next.contains("3"))
      }
      'remove - {
        val fetcher = new TestFetcher[String]
        val ps = PotStream(fetcher, Seq("1" -> Ready("test1"), "2" -> Ready("test2"), "3" -> Ready("test3"), "4" -> Ready("test4")))
        val ps1 = ps.remove("2")
        assert(!ps1.contains("2"))
        assert(ps1.get("1").next.contains("3"))
        val ps2 = ps1.remove("4")
        assert(!ps2.contains("2"))
        assert(ps2.get("3").next.isEmpty)
      ...
      'seq - {
        ...
        val seq0 = ps.seq
        assert(seq0.isEmpty)
        val ps1 = ps.append(Seq("1" -> Ready("test1"), "2" -> Ready("test2"), "3" -> Ready("test3"), "4" -> Ready("test4")))
        val seq = ps1.seq
        assert(seq.size == 4)
      ...
      'map - {
        val fetcher = new TestFetcher[String]
        val ps = PotStream(fetcher, Seq("1" -> Ready("test1"), "2" -> Ready("test2"), "3" -> Ready("test3"), "4" -> Ready("test4")))
        val ps1 = ps.map((k, v) => if(k != "4") v.pending() else v.fail(new Exception))
        assert(ps1("1").isPending)
        assert(ps1("2").isPending)
        assert(ps1("3").isPending)
        assert(ps1("4").isFailed)
      }
    }
  }
}
